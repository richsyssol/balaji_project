...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['password'], $_POST['itemId'])) {
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $row['password'])) {
                ...
                // Incorrect password
                echo "<script>alert('Incorrect password. Please try again.'); window.location.href='mf.php';</script>";
            }
        } else {
            echo "<script>alert('No admin user found.'); window.location.href='mf.php';</script>";
        }
    }
    ...
    if (isset($_POST['confirmed'])) {
        ...
        $query = "UPDATE mf_entries SET deleted_at = NOW(), is_deleted = 1 WHERE id = ?";

        $stmt = $conn->prepare($query);
        $stmt->bind_param('i', $itemId);

        if ($stmt->execute()) {
            echo "<script>alert('Item deleted successfully.'); window.location.href='mf.php';</script>";
        } else {
            echo "<script>alert('Error deleting item.'); window.location.href='mf.php';</script>";
        }
    }
}
?>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form method="POST" action="mf-delete.php">
