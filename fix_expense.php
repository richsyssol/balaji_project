        ...
        <div class="ps-5">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">FIXED EXPENSES</li>
                </ol>
            </nav>
        </div>
       ...
       <!-- Single Search Form -->
        <form method="POST" class="p-3">
            <div class="row">
                ...
                <!--php code for show dynamic option in dropdown-->
                <?php
                    ...
                    $query = "SELECT DISTINCT expense_type FROM fix_expense";
                    ...
                    $query = "SELECT DISTINCT mv_num FROM fix_expense";
                    $result = $conn->query($query);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $mv_num[] = $row['mv_num'];
                    ...
                    $query = "SELECT DISTINCT vehicle_type FROM fix_expense";
                    $result = $conn->query($query);
                        if ($result && $result->num_rows > 0) { // Check for valid $result and rows
                            while ($row = $result->fetch_assoc()) {
                                $vehicle_type[] = $row['vehicle_type']; // Use $vehicle_type here
                        }
                    } 
                ?>
                    ...
                    <select name="search_query" id="search_query" class="form-control">
                        ...
                        <?php foreach ($expense_types as $type): ?>
                            <option value="<?= htmlspecialchars($type) ?>" 
                                <?= isset($search_query) && $search_query === $type ? 'selected' : '' ?>>
                ...
                <div class="col-md-2 field">
                    <label for="search_query" class="form-label">Search by MV Number:</label>
                    <select name="mv_number" id="search_query" class="form-control">
                        <option value="">All</option>
                        <?php foreach ($mv_num as $type): ?>
                            <option value="<?= htmlspecialchars($type) ?>" 
                                <?= isset($search_query) && $search_query === $type ? 'selected' : '' ?>>
                                <?= htmlspecialchars($type) ?>
                            </option>
                ...
                <div class="col-md-2 field">
                    <label for="search_query" class="form-label">Search by Vehicle:</label>
                    <select name="vehicle" id="search_query" class="form-control">
                        <option value="">All</option>
                ...
                <!-- <div class="col-md-3 field">
                     <label for="search_query" class="form-label">Search by MV No</label>
                    <input type="text" name="mv_number" class="form-control" placeholder="Search by MV Number" />
                </div> -->
        
                <?php
                    // Set default values: Start date is empty, End date defaults to today
                ...
                <div class="col-md-2">
                    ...
                    <div class="col-md-1">
                        <a href="fix_expense_form" type="submit" class="btn sub-btn1 mt-4">ADD NEW</a>
                    </div>
                    ...
                    <?php if (isset($_SESSION['role']) && $_SESSION['role'] == 'admin') : ?>
                        ...
                        <!-- <div class="col-md-2">
                            <a href="expense-finance" type="submit" class="btn sub-btn1 mt-4">Finance</a>
                        </div> -->
...
if (!empty($search_query) || !empty($start_date) || !empty($end_date) || !empty($mv_number) || !empty($vehicle)) {
    ...
    $sql = "SELECT * FROM `fix_expense` WHERE is_deleted = 0";
    $params = [];
    $param_types = '';
    ...
    $count_sql = "SELECT COUNT(*) as total, SUM(amount) as total_amount FROM `fix_expense` WHERE is_deleted = 0";
    $count_params = [];
    $count_param_types = '';
        ...
        <table class="table table-bordered my-5">
            <thead>
                <tr>
                    ...
                    <!--<th scope="col">Quantity Lit</th>-->
                    <!--<th scope="col">Mon/Yr</th>-->
...
if (isset($result) && $result->num_rows > 0) {
    $serial_number = ($current_page - 1) * $items_per_page + 1; // Initialize serial number
    while ($row = $result->fetch_assoc()) { ?>
        <tr>
            <th scope="row"><?php echo $serial_number++; ?></th>
            <td><?php echo htmlspecialchars(date('d/m/Y', strtotime($row['policy_date']))); ?></td>
            <td><?php echo htmlspecialchars($row['mv_num']) . "<br>" . htmlspecialchars($row['vehicle_type']); ?></td>
            <td><?php echo htmlspecialchars($row['amount']); ?></td>
            <td><?php echo htmlspecialchars($row['expense_type']); ?></td>
            <td class="action-col">
                <?php if ($_SESSION['role'] == 'admin') : ?>
                    <a href="fix_expense_form?action=edit&id=<?php echo $row['id']; ?>" class="text-decoration-none text-dark">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a> &nbsp;/&nbsp;
                    <a data-bs-toggle="modal" data-bs-target="#passwordModal" data-item-id="<?php echo $row['id']; ?>">
