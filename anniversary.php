...
function fetchAnniversary($startDate = null, $endDate = null)
    {
        global $conn;
        $query = "";
        ...
        if (!$startDate && !$endDate) {
            $query = "SELECT client_name, contact, anniversary_date, address, age FROM client WHERE 
                    MONTH(anniversary_date) = MONTH(CURRENT_DATE) AND 
                    DAY(anniversary_date) = DAY(CURRENT_DATE)";
        } else {
            ...
            if ($startDateFormatted <= $endDateFormatted) {
                $query = "SELECT client_name, contact, anniversary_date, address, age FROM client WHERE 
                        DATE_FORMAT(anniversary_date, '%m-%d') BETWEEN '$startDateFormatted' AND '$endDateFormatted'
                        ORDER BY DATE_FORMAT(anniversary_date, '%m-%d') ASC";
            } else {
                $query = "SELECT client_name, contact, anniversary_date, address, age FROM client WHERE 
                        (DATE_FORMAT(anniversary_date, '%m-%d') BETWEEN '$startDateFormatted' AND '12-31')
                        OR 
                        (DATE_FORMAT(anniversary_date, '%m-%d') BETWEEN '01-01' AND '$endDateFormatted')
                        ORDER BY DATE_FORMAT(anniversary_date, '%m-%d') ASC";
            }
        }
...
function sendAnniversaryWishes($contact, $client_name)
{
...
function sendWhatsAppMessage($contact, $client_name) {
    ...
    $data = [
        ...
        "template" => [
            ...
            "name" => "anniversarymsg", // Your approved template name
        ]
    ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submit'])) {
        ...
        $result = fetchAnniversary($startDate, $endDate);
    ...
    if ($action === 'send_message' || $action === 'send_whatsapp') {
        ...
        if (!empty($selectedClients) || !empty($customContact)) {
            $message = "";

            // Sending message to custom contact (if entered)
            if (!empty($customContact)) {
                $clientName = "Custom Client";
                $response = ($action === 'send_message') 
                    ? sendAnniversaryWishes($customContact, $clientName) 
            ...
            // Sending messages to selected clients in chunks
            foreach ($chunks as $chunk) {
                foreach ($chunk as $originalContact) {
                    $newContact = $updatedContacts[$originalContact] ?? $originalContact; // Use updated number if available
                    $clientName = $clientNames[$originalContact] ?? 'Unknown';

                    $response = ($action === 'send_message') 
                        ? sendAnniversaryWishes($newContact, $clientName) 
...
if (isset($_POST['generate_csv'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($admin_password, $hashed_password)) {
            ...
            fputcsv($output, ['Reg Num', 'Client Name', 'Contact', 'Anniversary Date', 'Age', 'Address']);

            // **Fetch only today's birthdays**
            $sql = "SELECT * FROM client WHERE MONTH(anniversary_date) = MONTH(CURDATE()) AND DAY(anniversary_date) = DAY(CURDATE())";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            while ($row = $result->fetch_assoc()) {
                fputcsv($output, [
                    ...
                    $row['client_name'],
                    $row['contact'],
                    date('d/m/Y', strtotime($row['anniversary_date'])),
                    $row['age'],
                    $row['address']
                ]);
            }
    ...
    if (isset($_POST['generate_pdf'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if (password_verify($admin_password, $hashed_password)) {
                ...
                $pdf->Cell(30, 10, 'Anniversary Date', 1);
                ...
                while ($item = $result->fetch_assoc()) {
                    ...
                    $pdf->Cell(30, 10, date('d/m/Y', strtotime($item['anniversary_date'])), 1);
...
<section class="d-flex pb-5">
    ...
    <!-- Birthday Datails -->
        ...
        <?php if (!empty($birthdays)) : ?>
            ...
            <h4>Send Anniversary Wishes For Today:</h4>
                        ...
                        <?php if (!empty($birthdays)): ?>
                            <?php foreach ($birthdays as $client): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php echo date('d/m/Y', strtotime($client['anniversary_date'])); ?>
                                    </td>
