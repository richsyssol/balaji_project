...
<section class="d-flex pb-5">
    ...
    <div class="container data-table p-5 ">
        <div class="ps-5">
            ...
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">MF/FINANCE</li>
              </ol>
            </nav>
        </div>
            ...
            <div class="row">
                <div class="col-md-4 field">
                    ...
                    <input type="text" name="search_query" class="form-control"
                        ...
                        placeholder="Search by Date, Name, Mobile" />
                </div>
                ...
                <div class="col-md-1">
                    <a href="mf" class="btn sub-btn1 mt-4">Reset</a>
                </div>
                ...
                <?php if (isset($_SESSION['role']) && $_SESSION['role'] == 'admin') : ?>
                    <div class="row justify-content-center align-item-center">
                        ...
                        <div class="col-md-1">
                            <button type="button" class="btn sub-btn1 mt-4" data-bs-toggle="modal" data-bs-target="#passwordModal2">
                                PDF
                            </button>
                        </div>
                        ...
                        <div class="col-md-2">
                            <a href="mf-finance" type="submit" class="btn sub-btn1 mt-4">Finance</a>
                        </div>

                        <div class="col-md-2">
                            <a href="mf-remind" type="submit" class="btn sub-btn1 mt-4">Remind Msg</a>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
        ...
        if (!empty($search_query) || !empty($status) || !empty($start_date) || !empty($end_date)) {
            ...
            $sql = "SELECT * FROM `mf_entries` WHERE is_deleted = 0";
            $params = [];
            $param_types = '';
        
            // Handle Search Query
            if (!empty($search_query)) {
                $sql .= " AND (client_name LIKE ? OR contact LIKE ? OR mf_type LIKE ?)";
            ...
            // Fetch total number of records and total amounts for pagination
                ...
                if (!empty($search_query)) {
                    $count_sql .= " AND (client_name LIKE ? OR contact LIKE ? OR mf_type LIKE ?)";
        ...
        if (isset($_POST['generate_pdf'])) {
            ...
            $pdf->Cell(30, 10, 'Insurance Opt', 1);
            $pdf->Cell(30, 10, 'MF Opt', 1);
            ...
            if ($result->num_rows > 0) {
                while ($item = $result->fetch_assoc()) {
                    ...
                    $pdf->Cell(30, 10, $item['mf_type'], 1);
                    $pdf->Cell(30, 10, $item['insurance_option'], 1);
                    $pdf->Cell(30, 10, $item['mf_option'], 1);
            ...
            $pdf->Output('D', 'mf-report.pdf'); // Force download
            exit();
        }
    ...
    <div id="reportSection"> 
        ...
        <div class="heading">
            <?php
                ...
                echo "<h1 class='text-center'>MF REPORT FOR $formatted_start_date TO $formatted_end_date </h1>"
        ...
        <table class="table table-bordered my-5">
            <thead>
                <tr>
                    ...
                    <th scope="col">Insuarance Option</th>
                ...
                if (isset($result) && $result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['mf_type']); ?></td>
                            <td>
                                <?php echo htmlspecialchars($row['insurance_option']); ?> 
                                <?php echo htmlspecialchars($row['mf_option']); ?>
                            </td>
                            ...
                            <td class="action-col">
                                <?php if (isset($_SESSION['role']) && $_SESSION['role'] == 'admin') : ?>
                                    <a href="mf-form.php?action=edit&id=<?php echo $row['id']; ?>" class="text-decoration-none text-dark" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination neumorphic-pagination">
                    ...
                    <li class="page-item <?= ($items_per_page === 'all') ? 'active' : '' ?>">
                        <a class="page-link" href="?page=1&items_per_page=all&search_query=<?= urlencode($search_query) ?>&status=<?= urlencode($status) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&sort=<?= urlencode($sort) ?>">Show All</a>
                    </li>

                    <?php if ($current_page > 1) : ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?= $current_page - 1 ?>&items_per_page=<?= urlencode($items_per_page) ?>&search_query=<?= urlencode($search_query) ?>&status=<?= urlencode($status) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&sort=<?= urlencode($sort) ?>" aria-label="Previous">
                    ...
                    <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                        <li class="page-item <?= ($i === $current_page) ? 'active' : '' ?>">
                            <a class="page-link" href="?page=<?= $i ?>&items_per_page=<?= urlencode($items_per_page) ?>&search_query=<?= urlencode($search_query) ?>&status=<?= urlencode($status) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&sort=<?= urlencode($sort) ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>

                    <?php if ($current_page < $total_pages) : ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?= $current_page + 1 ?>&items_per_page=<?= urlencode($items_per_page) ?>&search_query=<?= urlencode($search_query) ?>&status=<?= urlencode($status) ?>&start_date=<?= urlencode($start_date) ?>&end_date=<?= urlencode($end_date) ?>&sort=<?= urlencode($sort) ?>" aria-label="Next">
...
<!-- Password Verification for delete Modal -->
<div class="modal fade" id="passwordModal" tabindex="-1" aria-labelledby="passwordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="verificationForm" action="mf-delete.php" method="POST">
